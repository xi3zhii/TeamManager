...
[![Test TeamM - Merge to Main](https://github.com/teamm-project/teamm/actions/workflows/test_merge.yml/badge.svg)](https://github.com/teamm-project/teamm/actions/workflows/test_merge.yml)

## Developing

* Clone the repo.
* Set up .env files in top level project directory and `frontend` subfolder (`.env.example.*` files are provided)
* Follow the README to install dockers.
* setup pre-commit (install: pip install pre-commit && pre-commit install)
...
```
#### Add a pre-commit configuration (if it doesn't already exist):
...
```
#### (Optional) To simply run pre-commit against all the files:
```console
$ pre-commit run --all-files
```
It's usually a good idea to run the hooks against all of the files when adding new hooks (usually pre-commit will only run on the changed files during git hooks)

#### Updating hooks automatically (run often!):
...
#### Information about creating new hooks:
...
$ docker-compose up -d
```
To run docker-compose using custom settings
...
```Bash
docker-compose exec backend /teamm/tests-start.sh
```
...
That `/teamm/tests-start.sh` script just calls `pytest` after making sure that the rest of the stack is running. If you need to pass extra arguments to `pytest`, you can pass them to that command and they will be forwarded.

For example, to stop on first error:

```bash
docker-compose exec backend bash /teamm/tests-start.sh -x
```
...
```bash
docker-compose exec backend bash /teamm/tests-start.sh --cov-report=html
```
...
In that case, you will need to use a fake local domain (`dev.teamm.online`) and make your computer think that the domain is is served by the custom IP (e.g. `192.168.99.150`).

If you used the default CORS enabled domains, `dev.teamm.online` was configured to be allowed. If you want a custom one, you need to add it to the list in the variable `BACKEND_CORS_ORIGINS` in the `.env` file.

* Open your `hosts` file with administrative privileges using a text editor:
  * **Note for Windows**: If you are in Windows, open the main Windows menu, search for "notepad", right click on it, and select the option "open as Administrator" or similar. Then click the "File" menu, "Open file", go to the directory `c:\Windows\System32\Drivers\etc\`, select the option to show "All files" instead of only "Text (.txt) files", and open the `hosts` file.
  * **Note for Mac and Linux**: Your `hosts` file is probably located at `/etc/hosts`, you can edit it in a terminal running `sudo nano /etc/hosts`.

* Additional to the contents it might have, add a new line with the custom IP (e.g. `192.168.99.150`) a space character, and your fake local domain: `dev.teamm.online`.

The new line might look like:

```
192.168.99.100    dev.teamm.online
```

* Save the file.
  * **Note for Windows**: Make sure you save the file as "All files", without an extension of `.txt`. By default, Windows tries to add the extension. Make sure the file is saved as is, without extension.

...that will make your computer think that the fake local domain is served by that custom IP, and when you open that URL in your browser, it will talk directly to your locally running server when it is asked to go to `dev.teamm.online` and think that it is a remote server while it is actually running in your computer.

To configure it in your stack, follow the section **Change the development "domain"** below, using the domain `dev.teamm.online`.

After performing those steps you should be able to open: http://dev.teamm.online and it will be server by your stack in `localhost`.

Check all the corresponding available URLs in the section at the end.

### Change the development "domain"

If you need to use your local stack with a different domain than `localhost`, you need to make sure the domain you use points to the IP where your stack is set up. See the different ways to achieve that in the sections above (i.e. using Docker Toolbox with `local.dockertoolbox.tiangolo.com`, using `localhost.tiangolo.com` or using `dev.teamm.online`).
...
```
### Frontend tests
To test frontend run (tests show coverage by default):
...
```bash
STACK_NAME=stage-teamm-online sh ./scripts/deploy.sh
```
...
services:
  db:
    ...
    deploy:
      placement:
        constraints:
          - node.labels.teamm-online.app-db-data == true
```
...
```bash
docker node update --label-add teamm-online.app-db-data=true dog.example.com
```

* Then you need to do the same for each stack version you have. For example, for staging you could do:

```bash
docker node update --label-add stage-teamm-online.app-db-data=true cat.example.com
```
...
* Set these environment variables:
  * `DOMAIN=teamm.online`
  * `TRAEFIK_TAG=teamm.online`
  * `STACK_NAME=teamm-online`
  * `TAG=prod`
...
Frontend: https://teamm.online

Backend: https://teamm.online/api/

Automatic Interactive Docs (Swagger UI): https://teamm.online/docs

Automatic Alternative Docs (ReDoc): https://teamm.online/redoc

~~PGAdmin: https://pgadmin.teamm.online~~

Flower: https://flower.teamm.online

### Staging URLs

Staging URLs, from the branch `master`.

Frontend: https://stage.teamm.online

Backend: https://stage.teamm.online/api/

Automatic Interactive Docs (Swagger UI): https://stage.teamm.online/docs

Automatic Alternative Docs (ReDoc): https://stage.teamm.online/redoc

~~PGAdmin: https://pgadmin.stage.teamm.online~~

Flower: https://flower.stage.teamm.online
...
Frontend: http://dev.teamm.online

Backend: http://dev.teamm.online/api/

Automatic Interactive Docs (Swagger UI): https://dev.teamm.online/docs

Automatic Alternative Docs (ReDoc): https://dev.teamm.online/redoc

~~PGAdmin: http://dev.teamm.online:5050~~

Flower: http://dev.teamm.online:5555

Traefik UI: http://dev.teamm.online:8090
